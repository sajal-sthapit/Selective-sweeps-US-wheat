...
# Start Here (for reanalysis - not create rehh format. The portion to create rehh format can be put into a different script file)
From this portion, we will start by loading the g.poly output from the previous chunk to run our analysis

# Load genotypes, varieties, and SNP types
In this chunk we create a new tibble snp.order with information on allele types for each SNP.
```{r load libraries, genotypes, and populations}
rm(list = ls())
gc()
...
# Scan population function to calculate EHH and iHS
...
# Calcuate genome wide iHS and scans for the entire panel, spring, and winter populations.
This chunk has one code set repeated 3 times for populations based on habit: both, spring, and winter as each sub-population has different sets of ancestral populations. If we were to use major/minor alleles then we can run the same code set for all the populations.
...
g <- map(.x = pop[35:79], .f = ~ cbind(geno$nuc[1:6], geno$nuc[.x]))
...
map(.x = chrm,
    .f = ~ filter(geno$nuc[1:6], Chrom == .x) %>% 
                 select(SNPid, Chrom, pos) %>% 
                 write_delim(path = str_c("rehh_files/map/", .x, ".inp"), col_names = FALSE, delim = " ") )

# run scan_hh for all populations to calcuate iHH, iES, and inES
res <- map(.x = g, .f = ~ scan_populations(inp = .x, chrm = chrm, param = param))
